...
function UnitHealthFrameMixin:Update()
    ...
    if allIncomingHeal > 0 then
        ...
        if allIncomingHeal + cur > max then allIncomingHeal = max - cur; end
    ...
    if class == "MONK" and spec == 1 then
        self:SetScript("OnUpdate", function()
            ...
            if stagger > 0 then
                ...
                local offset = (cur / max) * self:GetWidth();
                overlayDetails.anchor = PandaUICore:anchor("LEFT", "RIGHT",
                                                           -offset, 0);
            else
...
function PandaUIUnits:UnitHealthFrame(unit)
