...
local function MakePowerInfo(primary, secondary)
  ...
  if secondary then
    info.secondary = MakeSinglePowerInfo(secondary)
  end

  function info:GetSecondaryLabel()
    if self.secondary then
      return self.secondary.label
    end
    return nil
  end
...
function GetPowerInfo(class, spec)
  ...
  if classPowers[class] and classPowers[class][spec] then
    ...
    print("Class ", class, " with spec ", spec, " not configured. Using defaults.")
  end
...
local function PositionPrediction(frame, max, current)
  if not frame.predictedPowerCost then
    return
  end
  ...
  frame.details.anchor = PandaUICore:anchor("RIGHT", "RIGHT", -missingPercent * parentWidth, -1)
...
function PandaUIPlayer:PlayerPowerFrame()
  ...
  return {
    ...
    children = {
      ...
      -- }),
      PandaUICore:Merge(
        PandaUIUnits:UnitPowerFrame("player", secondary),
        {
          name = "SecondaryPower",
          ref = "secondaryPower",
      ...
      PandaUICore:Merge(
        PandaUIUnits:UnitPowerFrame("player"),
        {
          name = "PrimaryPower",
          ref = "primaryPower",
          layout = {parts = 2}
